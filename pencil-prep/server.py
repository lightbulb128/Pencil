...
if __name__ == "__main__":
    ...
    input_shape = (64, 3, 56, 56)
    model_name, model_torch = "", torch.nn.Conv2d(3, 64, 3)
    ...
    print("Prepare time =", time.time() - timed)
    ...
    for step in range(steps):
        ...
        for i, layer in enumerate(model.items):
            print(f"Forward layer {i} ", end="")
            layer_timed = time.time()
            outputs_a = layer.forward(outputs_a)
            outputs_reconstructed = cryp.field_mod(outputs_a + comm.recv())
            print("time = {0:.5f} ({1})".format(time.time() - layer_timed, layer))
