...
if __name__ == "__main__":
    ...
    model_name, model_torch = "", torch.nn.Sequential( # input (3, 32, 32)
    ...
    for step in range(steps):
        ...
        for i in range(1, len(outputs_list)):
            ...
            print(f"i={i}", "D(y) ", absmax(oreshaped - output_tensor_list[i].detach().numpy()), "[ max", absmax(output_tensor_list[i].detach().numpy()), ']', model.items[i-1])
            # print(oreshaped)
